...
namespace STRAFTATBattleRoyale
{
    ...
    // Respaldo para clientes: garantiza que Reset() se llame en todos
    [HarmonyPatch(typeof(PauseManager), nameof(PauseManager.InvokeBeforeSpawn))]
    static class Patch_InvokeBeforeSpawn
    {
        [HarmonyPostfix]
        static void Postfix()
        {
            Plugin.Log.LogInfo("[BR] InvokeBeforeSpawn detectado → Reset()");
            BattleRoyaleZone.Reset();
        }
    }
}
